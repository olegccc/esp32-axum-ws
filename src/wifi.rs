...
impl<'a> WifiConnection<'a> {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        loop {
            info!("Wifi: connecting to SSID '{}'...", self.state.ssid);
            ...
            *self.state.ip_addr.write().await = ip_info.ok().map(|i| i.ip);
            info!("Wifi: connected to '{}': {ip_info:#?} {}", self.state.ssid, self.state.ip_addr().await.unwrap());
